...
public class ContentNavigationService implements ResourceContainer {
	...
	public JsonMenu getNavigation() {
		...
		try {
			...
			List<Node> rawNodes = wcmComposer.getContents(repository, workspace, "", filters, WCMCoreUtils.getUserSessionProvider());
			*/
	...
	private List<JsonMenuNode> getChildNodes(NavigableNode parentNavigableNode, List<NavigableNode> allNodes) throws RepositoryException {
		...
		// TODO see why Utils.isViewable(navigableNode.getNode()) does not work.
		// It always returns true. This methods calls the checkPermission method of NodeImpl
		// which calls session.getUserState().getIdentity() to get the user's identity, but this method 
		// does not return the current user identity. It always return the userId "__system", who has 
		// permission everywhere (see org.exoplatform.services.jcr.access.AccessManager). Why ?? Because we are in the rest context ?
		//SessionImpl session = (SessionImpl) parentNavigableNode.getNode().getSession();
		...
		if(userState != null) {
			...
			userIdentity = new Identity(SystemIdentity.ANONIM);
		}
		...
		while (itAllNodes.hasNext()) {
			...
			// check permissions
			if (session.getAccessManager().hasPermission(((NodeImpl)navigableNode.getNode()).getACL(), PermissionType.READ, userIdentity)) {
				...
				if(nodePath.length() > parentNodePath.length() && nodePath.indexOf(parentNodePath) == 0) {
					...
					// we can have symlink to this node located under the current node
	...
	private JsonMenuNode buildNode(NavigableNode navigableNode) throws RepositoryException {
		...
		if(navigableNode.isClickable()) {
			...
			// path pattern is different if we display content dynamically in SCV or CLV...
	...
	public class JsonMenu {
		...
		public String toString() {
			...
			// sb.append("menu:{").append("label: ").append(label).append(" ").append("uri: ").append(uri).append(" ");
			// sb.append(" ").append("} ");
			return sb.toString();
		}
		*/
	...
	public class JsonMenuNode implements Comparable<JsonMenuNode> {
		...
		public String toString() {
			StringBuffer sb = new StringBuffer();
			sb.append("label: ").append(label).append(" ").append("uri: ").append(uri).append(" ");
			return sb.toString();
		}
		*/
