 ...
 * May 16, 2009  
 */
public class PermissionListAccess implements ListAccess<Permission> {

  private final List<Permission> list;

  PermissionListAccess(List<Permission> list) {
    
    if (list == null) {
      this.list = new ArrayList<Permission>();
    } else {
      this.list = list;
    }
  }

  public Permission[] load(int index, int length) throws Exception, IllegalArgumentException {
    if (index < 0)
