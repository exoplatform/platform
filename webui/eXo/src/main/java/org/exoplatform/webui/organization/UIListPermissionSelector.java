...
public class UIListPermissionSelector extends UISelector<String[]> {
	...
	public UIListPermissionSelector() throws Exception {
		UIFormCheckBoxInput<Boolean> uiPublicMode = new UIFormCheckBoxInput<Boolean>(
				"publicMode", null, false);
		...
		uiGrid.configure("expression", new String[] { "groupId", "membership" },
				new String[] { "Delete" });
		UIFormPageIterator uiIterator = (UIFormPageIterator) uiGrid
				.getUIPageIterator();
		uiIterator
				.setPageList(new LazyPageList(new PermissionListAccess(null), 10));
		addChild(uiIterator);
		uiIterator.setRendered(false);
		UIFormPopupWindow uiPopup = addChild(UIFormPopupWindow.class, null, null);
		uiPopup.setWindowSize(540, 0);

		UIGroupMembershipSelector uiMembershipSelector = createUIComponent(
				UIGroupMembershipSelector.class, null, null);
		uiMembershipSelector.setId("ListPermissionSelector");
		uiMembershipSelector.getChild(UITree.class).setId(
				"TreeListPermissionSelector");
		uiMembershipSelector.getChild(UIBreadcumbs.class).setId(
				"BreadcumbsListPermissionSelector");
	...
	public void clearGroups() throws Exception {
		...
		uiIterator
				.setPageList(new LazyPageList(new AccessGroupListAccess(list), 10));
	}
	...
	public UIListPermissionSelector setValue(String[] permissions)
			throws Exception {
		List<Object> list = new ArrayList<Object>();
		setPublicMode(false);
	...
	@SuppressWarnings("unchecked")
	public void setMembership(String groupId, String membershipType)
			throws Exception {
	...
	public void setPublicMode(boolean mode) throws Exception {
		...
		if (publicMode_)
			uiGrid.getUIPageIterator().setPageList(
					new LazyPageList(new AccessGroupListAccess(null), 10));
	}
	...
	static public class DeleteActionListener extends
		...
		public void execute(Event<UIListPermissionSelector> event) throws Exception {
			String permission = event.getRequestContext().getRequestParameter(
					OBJECTID);
			UIListPermissionSelector uiPermissions = event.getSource();
			UIPageIterator pageIterator = uiPermissions.getChild(UIFormGrid.class)
					.getUIPageIterator();
			...
			event.getRequestContext()
					.addUIComponentToUpdateByAjax(uiForm.getParent());
		}
	}
	...
	static public class ChangePublicModeActionListener extends
			EventListener<UIListPermissionSelector> {
		public void execute(Event<UIListPermissionSelector> event) throws Exception {
			UIListPermissionSelector uicom = event.getSource();
			UIFormCheckBoxInput<Boolean> uiPublicModeInput = uicom
					.getChildById("publicMode");
			uicom.setPublicMode(uiPublicModeInput.isChecked());
			uicom.setRendered(true);
			UIForm uiForm = uicom.getAncestorOfType(UIForm.class);
			UIPermissionSelector uiPermission = uiForm
	...
	static public class EmptyIteratorValidator implements Validator {
		public void validate(UIFormInput uiInput) throws Exception {
			...
			if (uiInputContainer.isPublicMode())
				return;
			UIFormPageIterator uiInputIterator = uiInputContainer
					.findFirstComponentOfType(UIFormPageIterator.class);
			if (uiInputIterator.getAvailable() < 1) {
				...
				throw new MessageException(new ApplicationMessage(
						"EmptyIteratorValidator.msg.empty", args, ApplicationMessage.INFO));
			}
		}
